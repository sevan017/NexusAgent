...
def get_config() -> KamaConfig:
    ...
    # .env 必须在读取 KAMA_CONFIG 之前加载，以便 .env中KAMA_CONFIG 能影响TOML路径
    load_dotenv(".env", override=False)
    # 解析配置文件路径：环境变量优先，否则用默认路径，并展开 ~
    config_path = Path(os.environ.get("KAMA_CONFIG", _DEFAULT_CONFIG_PATH)).expanduser()

    # TOML 文件缺失时静默跳过，解析失败则直接退出
...
# 将 TOML 数据覆盖到配置对象，未知键或类型错误直接退出
def _apply_toml(config: KamaConfig, data: dict[str, Any]) -> None:
    # 拒绝未知的顶级 section（同种校验模式，后续 [core]、[logging] 同理）
    ...
    if "core" in data:
        ...
        if "host" in core:
            val = core["host"]
            # 类型校验：与下方 port/int、logging 字段均为同模式
    ...
    if "logging" in data:
        ...
        # 遍历已知 logging 键，统一做 str 校验并 setattr 写入
...
# 用KAMA_* 环境变量覆盖 config 中对应的字段(若变量已设置)
def _apply_env(config: KamaConfig) -> None:
    # 每个环境变量遵循相同模式：读取 → 非空则覆盖；port 额外做 int 转换
